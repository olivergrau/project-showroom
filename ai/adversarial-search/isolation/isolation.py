...
class Isolation(NamedTuple('Isolation', [('board', int), ('ply_count', int), ('locs', int)])):
    ...
    # 
    #     # Provide information about the current player, action, and position
    #     print(f"result: Player {self.player()} is attempting to move.")
    #     print(f"result: Current location: {player_location}")
    #     print(f"result: Action: {action}")
    # 
    #     assert player_location is None or action in _ACTIONSET, \
    #         f"result: Error: Action {action} is not a valid action from the set {list(Action)}."
    # 
    #     if player_location is None:
    #         player_location = 0
    # 
    #     # Calculate the new location of the player after the action
    #     target_location = int(action) + player_location
    #     print(f"result: Target location: {target_location}")
    # 
    #     # Check if the target cell is blocked
    #     if not (self.board & (1 << target_location)):
    #         # Print the board configuration and relevant details
    #         debug_state = DebugState.from_state(self)
    #         print("result: Current board configuration:")
    #         print(debug_state)
    #         raise RuntimeError(f"Invalid move: target cell at location {target_location} is blocked."
    #                            f"\nPlayer: {self.player()}"
    #                            f"\nCurrent location: {player_location}"
    #                            f"\nAction: {action}"
    #                            f"\nTarget location: {target_location}")
    # 
    #     # Update the board to block the cell after the move
    #     board = self.board ^ (1 << target_location)
    #     locs = (self.locs[0], target_location) if self.player() else (target_location, self.locs[1])
    # 
