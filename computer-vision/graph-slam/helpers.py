...
def make_data(N, num_landmarks, world_size, measurement_range, motion_noise,
    ...
    while not complete:
        ...
        data = []
        true_path = []  # To store the true path of the robot
        movements = []  # To store movements for debugging
        ...
        for k in range(N):
            ...
            # store the true position before the move
            ...
            # Append the movement to the debug list
            movements.append([dx, dy])

        # check if the run is complete
        complete = (sum(seen) == num_landmarks)

    # Print movements if the run is complete and debug is enabled
    if debug:
        print("Movements for complete run:")
        for move in movements:
            print(f"dx: {move[0]:.2f}, dy: {move[1]:.2f}")
