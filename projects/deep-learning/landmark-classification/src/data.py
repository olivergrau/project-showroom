...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose(
            [
                # Scale the image so that the shortest side is between 256 and 480 pixels
                transforms.Resize((256, 480)),
                
                transforms.RandomResizedCrop((224, 224), scale=(0.8, 1.0)),  # Keep at least 80% of the image
        ...
        "valid": transforms.Compose(
            [
                transforms.Resize((256, 480)),  # scale shortest side to 256 pixels
                transforms.CenterCrop((224, 224)),  # center crop with 224x224 pixels
                transforms.ToTensor(),
                transforms.Normalize(mean, std)
            ]
        ),
    ...
    data_loaders["train"] = torch.utils.data.DataLoader(
        ...
        shuffle=False, # because we use a train sampler
    )
    data_loaders["valid"] = torch.utils.data.DataLoader(
        ...
        num_workers=num_workers,
        shuffle=False, # we use a valid sampler, but is that a good idea to shuffle the records in the valid set too?
    )
