...
class StateMachine(Generic[StateSchema]):
    def __init__(self, state_schema: Type[StateSchema], logger: logging.Logger = None):
    ...
    def run(self, state: StateSchema, resource: Resource = None):
        ...
        while current_step_id:
            ...
            if isinstance(step, Termination):
                self.logger.debug(f"[StateMachine] Terminating: {current_step_id}")
                break
            ...
            if isinstance(step, EntryPoint):
                self.logger.debug(f"[StateMachine] Starting: {current_step_id}")
            else:
                self.logger.debug(f"[StateMachine] Executing step: {current_step_id}")
