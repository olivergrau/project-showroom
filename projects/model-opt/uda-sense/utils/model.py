"""
udasense Project: Model Utilities Module
...
try:
    ...
    GraphModule = None  # type: ignore[attr-defined]
...
def load_model(
    ...
    **model_kwargs,
    ...
    Supports:
      - TorchScript archives (.pt / .jit / .ts) via torch.jit.load
      - state_dict checkpoints via torch.load + model_class(**model_kwargs)

    Args:
        path: Path to saved model file.
        device: Device to load model on ("cpu" or "cuda").
        model_class: Model class to instantiate for state_dict checkpoints.
        model_kwargs: Keyword args for model_class constructor.

    Returns:
        Loaded model (nn.Module or torch.jit.RecursiveScriptModule).
    """
    ...
    # 1) TorchScript fast path based on extension
    if ext in {".pt", ".jit", ".ts"}:
        try:
            ...
            raise RuntimeError(
                f"Failed to load TorchScript model from '{path}'. "
                f"Original error: {e}"
            ) from e

    # 2) Otherwise try torch.load (state_dict or pickled module)
    try:
        loaded = torch.load(path, map_location=map_location)
    except Exception as e:
        # Sometimes files are TorchScript but have non-.pt extension
        try:
            m = torch.jit.load(path, map_location=map_location)
            m.eval()
            return m
            ...
            raise RuntimeError(
                f"Failed to load model from '{path}' using torch.load or torch.jit.load. "
                f"Original torch.load error: {e}"
            ) from e

    # 3) If it's a GraphModule / module-like object (pickled), return it
    if isinstance(loaded, nn.Module):
        loaded = loaded.to(device) if device else loaded
        loaded.eval()
        return loaded

    # 4) If it's a plain state_dict, load into a fresh model instance
    if isinstance(loaded, dict) and "state_dict" not in loaded:
        if model_class is None:
            raise ValueError(
                "Checkpoint appears to be a state_dict, but no model_class was provided."
            )
    ...
    raise ValueError(
        ...
        "Expected TorchScript archive (.pt/.jit/.ts) or a plain state_dict."
    )
...
def save_model(model: nn.Module, path: str) -> None:
    ...
    Handles three cases:
      1) TorchScript ScriptModule: use torch.jit.save
      2) FX GraphModule: save whole module object
      3) Regular nn.Module: save state_dict
    """

    # 1) TorchScript model
    if isinstance(model, torch.jit.ScriptModule):
        # TorchScript has its own serialization format
        torch.jit.save(model, path)
        return

    # 2) FX optimized model
    if GraphModule is not None and isinstance(model, GraphModule):
        # For FX graphs we want to keep the transformed module structure
        torch.save(model, path)
        return

    # 3) Regular PyTorch model
    torch.save(model.state_dict(), path)
