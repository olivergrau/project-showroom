"""
udasense Project: Evaluation Metrics Module
...
#         bool_enable_cuda: Whether to measure inference time on CUDA if available
...
#     # Test on CUDA if available and enabled
...
#     #print(f"DEBUG[measure_inference_time]: Inference timing results: {results}")
...
def measure_inference_time(
    ...
    """Measure model inference time on both CPU and CUDA (if available) in a more stable way.
    
    Args:
        model: PyTorch model
        input_size: Shape of input tensor
        num_warmup: Number of warmup runs before timing (per device)
        num_runs: Number of timed runs per repeat
        enable_cuda: Whether to measure inference time on CUDA if available
        num_repeats: Number of independent timing measurements to average over
    ...
    # Helper to run repeated measurements on a single device
    def _measure_device(model_on_dev, device, label: str) -> Dict[str, float]:
        # Create dummy input
        ...
        with torch.inference_mode():
            for _ in range(num_warmup):
                _ = model_on_dev(dummy_input)

        # Multiple independent measurements to smooth out noise
        per_repeat_avg_ms: List[float] = []
        per_repeat_total_ms: List[float] = []
        ...
        with torch.inference_mode():
            for _ in range(num_repeats):
                ...
                per_repeat_total_ms.append(total_ms)
                per_repeat_avg_ms.append(avg_ms)

        # Aggregate statistics
        avg_time_ms = float(statistics.mean(per_repeat_avg_ms))
        median_time_ms = float(statistics.median(per_repeat_avg_ms))
        std_time_ms = float(statistics.pstdev(per_repeat_avg_ms)) if len(per_repeat_avg_ms) > 1 else 0.0

        # For total_time_ms we can report the mean total time over repeats
        total_time_ms = float(statistics.mean(per_repeat_total_ms))
    ...
    try:
        ...
        model_cpu = copy.deepcopy(model).to(cpu_device)
        model_cpu.eval()
        results["cpu"] = _measure_device(model_cpu, cpu_device, "cpu")
    except Exception as e:
        results["cpu"] = {}
        print(f"WARNING: CPU latency evaluation failed: {e}")
    ...
    if enable_cuda and torch.cuda.is_available():
        try:
            ...
            model_cuda = copy.deepcopy(model).to(cuda_device)
            model_cuda.eval()
            results["cuda"] = _measure_device(model_cuda, cuda_device, "cuda")
        except Exception as e:
            results["cuda"] = {}
            print(f"WARNING: GPU latency evaluation failed: {e}")
...
def evaluate_model_metrics(
    ...
    timing_metrics = measure_inference_time(model, input_size, enable_cuda=(device.type == 'cuda'))
