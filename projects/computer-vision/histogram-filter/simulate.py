...
class Simulation(object):
	def __init__(self, grid, blur, p_hit, start_pos=None):
		"""
        Initialize the simulation with a grid, blur factor, hit probability, and optional start position.
        """
		self.grid = grid
		...
		if not start_pos:
			self.true_pose = (self.height // 2, self.width // 2)  # Use // for integer division
		else:
