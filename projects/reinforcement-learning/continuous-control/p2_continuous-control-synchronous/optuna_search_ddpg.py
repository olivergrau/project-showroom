...
def coarse_objective(trial):
    ...
    exploration_noise = trial.suggest_float("exploration_noise", 0.5, 1.0)
    batch_size = trial.suggest_categorical("batch_size", [256, 512])
    critic_clip = trial.suggest_float("critic_clip", 1.0, 10.0)
    critic_weight_decay = trial.suggest_float("critic_weight_decay", 1e-6, 1e-4, log=True)

    use_prioritized_replay = trial.suggest_categorical("use_prioritized_replay", [False, True])
    ...
    # Ensure updates_per_block is at most 500
    updates_per_block = min(int(env_steps_per_update * update_multiplier), 500)
    ...
    use_reward_scaling = trial.suggest_categorical("use_reward_scaling", [False, True])
    reward_scaling_factor = trial.suggest_float("reward_scaling_factor", 1.0, 20.0)
    #use_reward_normalization = trial.suggest_categorical("use_reward_normalization", [False, True])
...
def fine_objective(trial):
    ...
    params = {
        ...
        "state_size": 33,
        "action_size": 4,
        "episodes": 1000,         # Use the same episode count for consistency.
        "max_steps": 1000,
        "batch_size": batch_size,
