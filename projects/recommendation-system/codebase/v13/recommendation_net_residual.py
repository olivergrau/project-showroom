...
class TransformerAttentionBlock(nn.Module):
    ...
    def forward(self, features):
        ...
        # Pass through Transformer (already has internal residual connections)
...
class ConfigurableRecommendationNetExperimental(nn.Module):
    """
    ...
    def __init__(
        ...
        super(ConfigurableRecommendationNetExperimental, self).__init__()
        ...
        # ----------------------------------------------------
        # 1. Dense layers for user and movie + skip projection
        # ----------------------------------------------------
        self.user_dense = nn.Linear(user_dim, dense_hidden)
        self.movie_dense = nn.Linear(movie_dim, dense_hidden)

        # [ADDED FOR RESIDUAL CONNECTIONS]
        # We add skip connections from raw user/movie profile -> dense output
        # so that user_emb_dense = user_dense(...) + user_skip(...)
        self.user_skip = nn.Linear(user_dim, dense_hidden)
        self.movie_skip = nn.Linear(movie_dim, dense_hidden)
        ...
        # 4. Final MLP (prediction head) with residual connection
        #    We'll combine everything into "combined_dim", then feed to a 2-layer MLP
        #    and add a skip projection from the combined_dim -> hidden_dim.
        ...
        # [ADDED FOR RESIDUAL CONNECTIONS]
        self.skip_proj = nn.Linear(combined_dim, dense_hidden)

        # Then output layer for rating
        self.out = nn.Linear(dense_hidden, 1)

        # Optional multitask output
        if self.multitask and genres_vocab_size > 0:
            self.genre_head = nn.Linear(dense_hidden, genres_vocab_size)
    ...
    def forward(self, user_profile, movie_profile):
        ...
        """
        # -------------------------------
        # 1. Dense transformations (+ skip)
        # -------------------------------
        user_emb_dense = self.activation(self.user_dense(user_profile))
        # [ADDED FOR RESIDUAL CONNECTIONS]
        user_emb_dense = user_emb_dense + self.activation(self.user_skip(user_profile))

        movie_emb_dense = self.activation(self.movie_dense(movie_profile))
        # [ADDED FOR RESIDUAL CONNECTIONS]
        movie_emb_dense = movie_emb_dense + self.activation(self.movie_skip(movie_profile))

        # Combine user and movie embeddings (simple addition or concatenation)
        ...
        if self.use_transformer:
            ...
            # The nn.TransformerEncoder has internal residual connections by default.
            attended_features = self.transformer_block(transform_input)

            # Concatenate the Transformer’s attended output
            combined_features = torch.cat([combined_features, attended_features], dim=1)

        # -------------------------------
        # 4. Final MLP for rating (+ skip)
        # -------------------------------
        hidden = self.final_mlp(combined_features)
        # [ADDED FOR RESIDUAL CONNECTIONS]
        hidden = hidden + self.skip_proj(combined_features)

        rating_pred = self.out(hidden)

        if not self.multitask:
            return rating_pred, None
        else:
            # Multi-task branch example: genre prediction
            genre_pred = self.genre_head(hidden)  # [batch_size, num_genres]
