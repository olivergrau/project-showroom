...
class HistoryTokenEmbedder(nn.Module):
    """
    Extends the idea of TokenEmbedder for user_history_tokens.

    user_history_tokens is a dict, each key has shape [B, max_history, property_dim].
    We want to produce a single embedding per history item => output shape [B, max_history, d_model].
    """
    def __init__(self, token_dims, d_model):
        super().__init__()

        # We'll store the order of keys (same as your "token_order" in TokenEmbedder).
        self.token_order = list(token_dims.keys())

        # Compute total input dimension by summing dims for each token
        total_input_dim = sum(token_dims.values())

        # A single linear layer to go from (sum of property_dim) -> (d_model)
        self.linear = nn.Linear(total_input_dim, d_model)

        self.d_model = d_model
        self.token_dims = token_dims

    def forward(self, user_history_tokens):
        """
        user_history_tokens: dict of Tensors, each shape [B, max_history, property_dim].
          e.g., {
             "movieId": [B, max_hist, 1],
             "budget":  [B, max_hist, 1],
             ...
             "tags":    [B, max_hist, 384],
             ...
        ...
        Returns:
          history_emb: [B, max_history, d_model]
        """

        # Step 1: Identify batch size and max_history by peeking at one entry
        sample_key = self.token_order[0]
        B, max_hist, _ = user_history_tokens[sample_key].shape

        # Step 2: Flatten each key from [B, max_hist, property_dim] -> [B*max_hist, property_dim]
        #         Then concatenate along dim=1
        flattened_list = []
        for token_name in self.token_order:
            tensor_for_key = user_history_tokens[token_name]  # shape [B, max_hist, property_dim]
            flattened_tensor = tensor_for_key.view(B * max_hist, -1)  # [B*max_hist, property_dim]
            flattened_list.append(flattened_tensor)

        # Step 3: Concatenate all keys => shape [B*max_hist, total_input_dim]
        concatenated = torch.cat(flattened_list, dim=1)  # [B*max_hist, total_input_dim]

        # Step 4: Project to d_model
        projected = self.linear(concatenated)  # [B*max_hist, d_model]

        # (Optional) If you want non-linear activation, you can do that here or in a small nn.Sequential
        # e.g. projected = torch.relu(self.linear(concatenated))

        # Step 5: Reshape back to [B, max_hist, d_model]
        history_emb = projected.view(B, max_hist, self.d_model)
...
class HistoryTransformer(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask):
        """
        x: [B, max_history, d_model]
        src_key_padding_mask: [B, max_history] with True=ignore, False=attend
        Returns: [B, d_model] after attention pooling.
        """

        # Pass through the transformer
        # shape out: [B, max_history, d_model]
        transformed = self.transformer(x, src_key_padding_mask=src_key_padding_mask)

        # Simple attention over time dimension
        attn_scores = self.attention_proj(transformed)  # [B, max_history, 1]
        attn_weights = torch.softmax(attn_scores, dim=1) # [B, max_history, 1]
        summary_emb = torch.sum(transformed * attn_weights, dim=1)  # [B, d_model]
...
class RecommendationNet(nn.Module):
    ...
    def __init__(
        ...
        # 1. Dense tokenization layers for user and movie profiles and user history
        ...
        # user_tokens(8) + movie_tokens(9) + 1 if fm_enabled
        ...
        # ---------------------------
        # (New) For User History
        # ---------------------------
        self.use_history_transformer = use_history_transformer
        if self.use_history_transformer:
            # A) Embedding for user history
            #    We might use the same token_dims as for movies if each history item is effectively a "movie profile".
            #    Or define a separate dict for them. Let's assume user_history_token_dims = movie_token_dims for now.
            self.user_history_embedder = HistoryTokenEmbedder(self.movie_token_dims, transformer_d_model)

            # B) Separate positional encoding for the history
            self.history_positional_encoding = PositionalEncoding(transformer_d_model)

            # C) The dedicated HistoryTransformer
            self.history_transformer = HistoryTransformer(
                d_model=transformer_d_model,
                nhead=transformer_nhead,
                dim_feedforward=transformer_feedforward,
                num_layers=transformer_layers
            ...
            self.user_history_embedder = None
    ...
    def forward(self, user_tokens, movie_tokens, user_history_tokens=None, user_history_mask=None):
        """
        Args:
            user_tokens (dict): Tokenized user features, shape [batch_size, user_seq_len, ...] after embedding
            movie_tokens (dict): Tokenized movie features, shape [batch_size, movie_seq_len, ...] after embedding
            user_history_tokens (dict or similar): User's past item interactions, 
                                                shape [batch_size, max_history, ...] in dict form
            user_history_mask (torch.Tensor): Binary mask for valid vs. padded history items, shape [B, max_history]
        Returns:
            rating_pred (torch.Tensor): Predicted rating or score, shape [B, 1]
            None: placeholder for optional multi-task output
        """
        ...
        # --------------------
        # 3. (NEW) User History Embedding & Transformer
        # --------------------
        history_emb = None
        if self.use_history_transformer and (user_history_tokens is not None):
            start_time = time.time()

            # A) Convert the dict of [B, max_hist, property_dim] into [B, max_hist, d_model]
            history_emb = self.user_history_embedder(user_history_tokens)  # [B, max_hist, d_model]

            # B) Positional encoding for the time dimension            
            history_emb = self.history_positional_encoding(history_emb)  # [B, max_hist, d_model]

            # C) Build a bool mask for the transformer: True = ignore padding
            #    user_history_mask has 1=valid, 0=pad => invert
            src_key_padding_mask = (user_history_mask == 0)  # shape [B, max_hist], bool

            # D) Pass through the dedicated HistoryTransformer => [B, d_model]
            B, max_hist, _ = history_emb.shape
            if max_hist == 0:
                # e.g., create a dummy vector of zeros for history_emb
                history_summary = torch.zeros(B, self.transformer_d_model, device=history_emb.device)
            else:
                with torch.cuda.amp.autocast(enabled=False):
                    history_summary = self.history_transformer(history_emb, src_key_padding_mask)
            # shape: [B, d_model] after attention pooling

            timings["History Transformer"] = time.time() - start_time

            # E) Integrate into user_emb
            #    For example, add to the first user token:
            user_emb[:, 0, :] += history_summary
