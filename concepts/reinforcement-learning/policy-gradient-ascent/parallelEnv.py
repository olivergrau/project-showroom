...
class parallelEnv(VecEnv):
    ...
    def reset(self):
        ...
        # Get the results from each remote
        results = [remote.recv() for remote in self.remotes]
        # If a result is a tuple (obs, info), take the observation only.
        observations = [res[0] if isinstance(res, (tuple, list)) and len(res) == 2 else res for res in results]
